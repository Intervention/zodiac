...
class ChineseNewYearCalculator
{
    /**
    ...
    /**
     * @return array{'day': array{int, int, int}, 'classname': string}
     */
    public static function newYear(int $year): array
    {
        [$month, $day, $classname] = self::$table[$year] ?? [null, null, null];

        if ($month === null || $day === null || $classname === null) {
            throw new RuntimeException('No new year data for ' . $year);
        }

        return ['day' => [$year, $month, $day], 'classname' => $classname];
    }
}
